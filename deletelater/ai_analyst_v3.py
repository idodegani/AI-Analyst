#!/usr/bin/env python3
"""
AI Agent Analyst V3 - Robust Version with Retry Logic and Structured Responses
"""
...
class AIAnalyst:
    """AI-powered data analyst with robust error handling and retry logic"""
    
    def __init__(self):
        print("🚀 Initializing AI Agent Analyst V3...")
        self.conn = duckdb.connect(':memory:')
        self.client = OpenAI()
        self.state = ConversationState()
        self.show_sql = True  # Always show SQL in V3
        self.max_retries = 1
        self.load_data()
        self.setup_workflow()
    ...
    def load_data(self):
        ...
        try:
            ...
            print(f"📊 Loaded {len(reservations_df):,} reservations and {len(reviews_df):,} reviews")
            ...
            stats = self.conn.execute("""
                SELECT 
                    AVG(r.fee_host_payout_usd) as avg_payout,
                    AVG(rv.overall_rating) as avg_rating
                FROM reservations r
                LEFT JOIN reviews rv ON r.reservation_id = rv.reservation_id
            """).fetchone()
            
            print(f"\n📈 Average payout: ${stats[0]:.2f}")
            print(f"⭐ Average rating: {stats[1]:.1f}" if stats[1] else "⭐ No ratings available")
            print()
...
IMPORTANT: Return ONLY the SQL query, nothing else."""
        ...
        if self.state.last_context and self._is_follow_up(question):
            context_msg = f"Previous query context: {self.state.last_context.previous_sql}"
    ...
    def execute_query(self, state: dict) -> dict:
        ...
        try:
            ...
            # Store context
            context = self.extract_context(self.state.current_question, sql_query)
            self.state.last_context = context
            self.state.sql_history.append(sql_query)
...
$474.66 for currency). If the result is a single value, state it clearly. If multiple results, summarize appropriately."""
            ...
            user_prompt = f"""Question: {self.state.current_question}
SQL Query: {sql_query}
...
Provide a natural language answer to the question based on these results."""
    ...
    async def ask(self, question: str) -> AnalystResponse:
        ...
        # Run workflow
        result = await self.app.ainvoke({"question": question})
        
        # Get response
        response = result.get("response")
        if isinstance(response, AnalystResponse):
            self.state.messages.append({"role": "assistant", "content": response.text_answer})
            return response
        else:
            # Fallback
...
def main():
    ...
    print("\n" + "="*60)
    print("💬 AI AGENT ANALYST V3 - Enhanced Chat Interface")
    print("="*60)
    print("""
📌 Features:
  • Natural language answers with SQL queries
  • Automatic retry on errors
  • Support for Q1-Q4 (quarters) and H1-H2 (half years)
    ...
    while True:
        try:
            ...
            if user_input.lower() == 'exit':
                ...
                print("""
📝 Example questions:
  • What is the average guest count for Q1 2025?
  • And for H1 2025? (follow-up)
  • What is the average review rating?
  • What is the average cleaniness rating for March 2025?
  • Give me 5 review IDs with overall rating above 6
  • What are the check-in dates for review_id 1345574968390852668?
  • Show me the total revenue by month for 2025
""")
                continue
                
            if user_input:
                # Run async function
                response = asyncio.run(analyst.ask(user_input))
                ...
                if response.status == QueryStatus.ERROR:
                    print(f"\n⚠️  Error Details: {response.error_message}")
